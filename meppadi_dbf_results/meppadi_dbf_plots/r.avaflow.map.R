...
#
# COPYRIGHT:    (c) 2013 - 2024 by the author
...
prefix <- 'meppadi_dbf_'  #prefix for file names
aflag <- 1  #control for additional output
tflag <- 0  #control for tsunami
model <- 1  #model
ymax0 <- 1288106.375000  #coordinates defining map boundaries
ymin0 <- 1276940.750000
xmin0 <- 614385.875000
xmax0 <- 625551.500000
cellsize <- 92.277847
tint <- 10.000000
...
maxhflow <- 6.221836
maxtflow <- 2669463.500000
maxpflow <- 7085573.000000
maxvflow <- 62.740971
maxtreach <- 58.418327
maxhtsun <- 0.000000
maxhentr <- 0.000000
...
if ( mult == 0 ) {
  ...
  for ( j in jrange ) {  # loop over all sets of maps to be created
    ...
    } else if ( j == 3 ) {  # for entrainment and deposition map
    ...
    if ( j == 0 && depdef == 1 ) {
        ...
        hdeposit1 <- reclassify(hdeposit0,c(-Inf,as.numeric(thresholdsh),0,as.numeric(thresholdsh),Inf,1))
        hdeposit <- t(as.matrix(hdeposit1))
  ...
  for ( ntimesteps in ntimemin:( ntimemax+1 )) { # loop over all time steps plus one for maps of maximum values
    ...
    if (model <= 3 || fill == 'iscore' || mstring == 'basechange' || mstring == 'htsun' || mstring == 'treach') {
    ...
    if (ortho1 != 'None') {
        ortho1r <- raster(ortho1)
        ortho2r <- raster(ortho2)
        ortho3r <- raster(ortho3)
        orthogray <- 153+(ortho1r+ortho2r+ortho3r)/7.5 #grayscale orthophoto
        orthogray[orthogray<153] <- 153
        orthogray[is.na(orthogray)==TRUE] <- 153
    } else if ( mult != 0 ) {
    ...
    if ( mult != 0 ) { # for multiple model runs
      ...
      a2 <- reclassify(mstringtr, c(-Inf,thrsm,0,thrsm,thrs1,1,thrs1,thrs2,2,thrs2,thrs3,3,thrs3,thrs4,4,thrs4,thrs5,5))
    ...
    } else if ( model <= 3 || j == 3 || (ntimesteps == ntimemax + 1 && j == 6 )) {
      ...
      if ( j == 3 ) { # for entrainment and deposition map:
        ...
        a2 <- reclassify(mstringtr * as.numeric(mconv2), c(-Inf,thrsm,0,thrsm,thrs1,1,thrs1,thrs2,2,thrs2,thrs3,3,thrs3,thrs4,4,thrs4,thrs5,5,thrs5,Inf,6))
    ...
    if (ortho1 != 'None') {
        plot(orthogray, legend.width = 1, col = gray((max(0, cellStats(orthogray, 'min')):min(255, 
            cellStats(orthogray, 'max')))/255), legend = FALSE, axes = FALSE, box = FALSE, 
            xlab = NA, ylab = NA, xlim = c(xmin, xmax), ylim = c(ymin, ymax), useRaster = TRUE)  #grayscale orthophoto
    } else {
    ...
    if (model < 7 || fill == 'iscore' || mstring == 'basechange' || mstring == 'htsun' || mstring == 'treach' ) {
    ...
    if (ctrlpts > 0) {
        ...
        par(new = TRUE)
        if ( fill == 'iscore' ) {
            inctrl <- paste(wkdir, prefix, 'results/', prefix, 'files/', prefix, 'ctrlpoints1.txt', 
                sep = '')
        } else {
    ...
    if (model == 7 && mstring != 'basechange' && mstring != 'htsun' && mstring != 'treach' && fill != 'iscore') {
        ...
        legend(x = xmax, y = ymax - ydiff/1.75, xjust = 0, legend = c('Phase 1', 'Phase 2', 'Phase 3'), 
